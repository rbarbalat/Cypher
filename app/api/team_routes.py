...
def get_user_teams():
    ...
    team_list = [
        {"id": team.id, "name": team.name, "image": team.image} for team in team_list
    ]
    return team_list
...
def create_channel(id):
    ...
    isOwnerOrAdmin = (
        TeamMembership.query.filter(TeamMembership.user_id == current_user.id)
        .filter(or_(TeamMembership.status == "admin", TeamMembership.status == "owner"))
        .filter(TeamMembership.team_id == id)
        .first()
    )
    ...
    if form.validate_on_submit():
        ...
        db.session.add(
            ChannelMembership(user=current_user, channel=channel, status="owner")
        )
        db.session.commit()
        return {
            "id": channel.id,
...
def add_member_to_team(id):
    ...
    tm = TeamMembership(user=current_user, team=team, status="member")
    db.session.add(tm)
    db.session.commit()
...
def delete_member_from_team(team_id, mem_id):
    ...
    tm = (
        TeamMembership.query.filter(TeamMembership.user_id == mem_id)
    ...
    # an owner or admin deletes a user
        ...
        team_mem.user.id
        for team_mem in TeamMembership.query.filter(TeamMembership.status != "member")
        .filter(TeamMembership.team_id == team_id)
        .all()
    ]:
        db.session.delete(tm)
